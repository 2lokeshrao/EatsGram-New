...
const LanguageModal = ({ modalVisible, setModalVisible, currentTheme, showCrossButton, dontClose }) => {
  ...
  async function determineInitialLanguage() {
    try {
      ...
      const storedLanguageCode = await AsyncStorage.getItem('EatsGram-language')
  ...
  async function onSelectedLanguage() {
    try {
      ...
      await AsyncStorage.setItem('EatsGram-language', languageCode)
      await AsyncStorage.setItem('EatsGram-language-name', languageVal)
